...
macro "title" {
	montageFolder = outputFolder + "/" + "montages";
	nchannels = 2;
	gfp_ch = "C1"
	DAPI_ch = "C2"
	
	File.makeDirectory(montageFolder);
	IJ.run("Make Montage Directory", "source="+ outputFolder +" destination="+montageFolder+" gray="+nchannels+" merge=1 g1="+gfp_ch+" g2="+DAPI_ch+" red=*None* green="+gfp_ch+" blue=*None* gray=*None* cyan="+DAPI_ch+" magenta=*None* yellow=*None* grout=8 dpi=300 include scale=10 scaling,1=0.069");
