...
def get_skel_results_table_header_cols():
    ...
    return [
        ...
        "NumberOfSlabVoxels",
        "AverageBranchLength",
        "NumberOfTriple points",
        "NumberOfQuadruple points",
        "MaximumBranchLength",
...
def analyze_skel(imp, output_parameters, out_path):
    ...
    for i in range(len(shortest_paths)):
        cumulative_shortestpaths_length = (
            cumulative_shortestpaths_length + shortest_paths[i]
        )
    ...
    output_parameters.update(
        {
            ...
            "Cumulative_ShortestPathLength": cumulative_shortestpaths_length,
            "Total_Donuts": num_donuts,
        }
    )
    # calculate totals / max and populate output parameters from the table
    columns = full_skel_results_table.getColumnHeadings()
    for colname in columns[1:11]:  # skip index and coordinates
        ...
        try:
            ...
            print(
                "Column "
                + colname
                + " not in output parameters, skipping total and max for this column"
            )
...
def calculate_footprint(imp, output_parameters):
    ...
    binary = Duplicator().run(
        imp, imp.getChannel(), imp.getChannel(), 1, slices, 1, frames
    )
    ...
    if binary.getNSlices() == 1:
        ...
        output_parameters["MitochondrialFootprint"] = (
            mito_footprint * imp_calibration.pixelDepth
        )
