//Open middle plane image
path = "/mnt/bigdisk1/Allie_S/Replicative_Age_Project/MRC-5_MiddlePlanes/Mar26_MitoLyso/";
for(i = 0; i < 40; i++) {
	rowcolfield = random_image();
	merge_ch(path,rowcolfield);
	name = getTitle();
	save("/mnt/bigdisk1/Allie_S/Replicative_Age_Project/MRC-5_MiddlePlanes/cellpose_test/" + name + ".tif");
}
...
function merge_ch(path,rowcolfield) {
	close("*");
	open(path + rowcolfield +"p04-ch1"+ "sk1fk1fl1.tiff");
	ch_1 = getTitle();
	
	open(path + rowcolfield +"p04-ch2"+ "sk1fk1fl1.tiff");
	ch_2 = getTitle();
	
	open(path + rowcolfield +"p04-ch3"+ "sk1fk1fl1.tiff");
	ch_3 = getTitle();
	
	open(path + rowcolfield +"p04-ch4"+ "sk1fk1fl1.tiff");
	ch_4 = getTitle();
